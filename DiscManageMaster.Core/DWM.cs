...
namespace DiscManageMaster.Core
{
    public class VistaApi
    {
        ...
        private static void EnableBlurBehindWindow(Control control, bool enable, IntPtr region, bool transitionOnMaximized)
        {
            DWM_BLURBEHIND blurBehind = new DWM_BLURBEHIND
                                            {
                                                dwFlags =
                                                    (DWM_BLURBEHIND.DWM_BB_ENABLE |
                                                     DWM_BLURBEHIND.DWM_BB_TRANSITIONONMAXIMIZED),
                                                fEnable = enable,
                                                fTransitionOnMaximized = transitionOnMaximized
                                            };

            if (enable && region.ToInt32() != 0)
            {
                blurBehind.dwFlags |= DWM_BLURBEHIND.DWM_BB_BLURREGION;
                blurBehind.hRegionBlur = region;
            }
        }

        public static void EnableBlurBehindWindow(Control control, bool enable)
        {
            Graphics g = Graphics.FromHwnd(control.Handle);
            //Once again dont paint the control black! Set it to black! So you can use "this".DoubleBuffered = true;
            control.BackColor = Color.Black;
            EnableBlurBehindWindow(control, enable, IntPtr.Zero, false);
        }
