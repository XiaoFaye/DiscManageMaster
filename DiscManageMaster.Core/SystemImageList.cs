...
namespace DiscManageMaster.Core
{
    ...
    public enum SystemImageListSize : int
    {
        ...
        ExtraLargeIcons = 0x2,
        /// <summary>
        /// Vista Icon Size (typically 256x256).
        /// Only available under Vista; under other OS the
        /// Large Icon ImageList is returned.
        /// </summary>
        VistaIcons = 0x4
    }
    ...
    public class SystemImageList : IDisposable
    {
        ...
        {
            if (!CommVar.IsVista && size == SystemImageListSize.VistaIcons)
                this.size = SystemImageListSize.ExtraLargeIcons;
            else
                this.size = size;
            create();
        }
        ...
        public void DrawImage(IntPtr hdc, int index, int x, int y, ImageListDrawItemConstants flags,
            int cx, int cy, Color foreColor, ImageListDrawStateConstants stateFlags,
            Color saturateColorOrAlpha, Color glowOrShadowColor)
        {
            IMAGELISTDRAWPARAMS pimldp = new IMAGELISTDRAWPARAMS();
            pimldp.hdcDst = hdc;
            pimldp.cbSize = Marshal.SizeOf(pimldp.GetType());
